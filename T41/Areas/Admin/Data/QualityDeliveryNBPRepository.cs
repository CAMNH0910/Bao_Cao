...
{
    public class QualityDeliveryNBPRepository
    {
        #region GetAllPOSCODE
        //Lấy mã bưu cục phát dưới DB Procedure Detail_DeliveryPosCode_Ems
        public IEnumerable<GETPOSCODENBP> GetAllPOSCODE(int zone)
        {
            List<GETPOSCODENBP> listGetPosCode = null;
            GETPOSCODENBP oGetPosCode = null;
            ...
            try
            {
                using (OracleCommand cm = new OracleCommand())
                {
                    ...
                    {
                        listGetPosCode = new List<GETPOSCODENBP>();
                        while (dr.Read())
                        {
                            oGetPosCode = new GETPOSCODENBP();
        ...
        public ReturnQualityNBP QUALITY_DELIVERY_DETAIL(int zone,int endpostcode,int routecode, int startdate, int enddate, int service)
        {
            DataTable da = new DataTable();
            MetaDataNBP _metadata1 = new MetaDataNBP();
            Convertion common = new Convertion();
            ReturnQualityNBP _returnQuality = new ReturnQualityNBP();
            ...
            List<QualityDeliveryDetailNBP> listQualityDeliveryDetail = null;
            QualityDeliveryDetailNBP oQualityDeliveryDetail = null;
            int a = 1;
            try
            {
                ...
                {
                   OracleCommand myCommand = new OracleCommand("kpi_detail_delivery_NBP.Detail_area_Ems", Helper.OraDCOracleConnection);
                   //xử lý tham số truyền vào data table
                    ...
                    if (dr.HasRows)
                    {
                        listQualityDeliveryDetail = new List<QualityDeliveryDetailNBP>();
                        while (dr.Read())
                        {
                            oQualityDeliveryDetail = new QualityDeliveryDetailNBP();
        ...
        #region QUALITY_DETAIL          
        public ReturnQualityNBP Quality_Delivery_Success6H_Detail(int endpostcode,int routecode, int startdate, int enddate, int service, int type)
        {
            DataTable da = new DataTable();
            MetaDataNBP _metadata1 = new MetaDataNBP();
            Convertion common = new Convertion();
            ...
            _returnQuality.MetaData1 = _metadata1;
            List<QualityDeliverySuccess5HDetailNBP> listQualityDeliverySuccess6HDetail = null;
            QualityDeliverySuccess5HDetailNBP oQualityDeliverySuccess6HDetail = null;
            try
            {
                // Gọi vào DB để lấy dữ liệu.
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    cmd.CommandText = Helper.SchemaName + "kpi_detail_delivery_NBP.Detail_Item_Ems";
                    ...
                    if (dr.HasRows)
                    {
                        listQualityDeliverySuccess6HDetail = new List<QualityDeliverySuccess5HDetailNBP>();
                        while (dr.Read())
                        {
                            oQualityDeliverySuccess6HDetail = new QualityDeliverySuccess5HDetailNBP();
        ...
        #region QUALITY_DETAIL          
        public ReturnQualityNBP Quality_Delivery_NoInformation_Detail(int endpostcode, int routecode, int startdate, int enddate, int service, int type)
        {
            DataTable da = new DataTable();
            MetaDataNBP _metadata1 = new MetaDataNBP();
            Convertion common = new Convertion();
            ...
            try
            {
                ...
                {
                    cmd.Connection = Helper.OraDCOracleConnection;
                    cmd.CommandText = Helper.SchemaName + "kpi_detail_delivery_NBP.Detail_Item_Ems_KTT";
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(new OracleParameter("v_EndPostCode", OracleDbType.Int32)).Value = endpostcode;
                    cmd.Parameters.Add(new OracleParameter("v_routecode", OracleDbType.Int32)).Value = routecode;
                    cmd.Parameters.Add(new OracleParameter("v_Service", OracleDbType.Int32)).Value = service;
